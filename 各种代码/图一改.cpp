				...
				for(int f = 0; f <= 500; f++){
					...
					if(w>=water[i+1]*2 && f>=food[i+1]*2 && water[i+1] < 10){
						for(int k = 1; k <= cnct[j][0]; k++){
							...
							if(mp[j] == 1){
								for(int m = 0; m <= 300-w; m++)
									for(int n = 0; n <= 500-f; n++){
										if(m * 10 + n*20 > dp[i][j][w][f] || 3*(m+w) + 2*(n+f) > weight) break;
							...
							if(mp[cnct[j][k]] == 1){
								for(int m = 0; m <= 30-w+water[i+1]*2; m++)
									for(int n = 0; n <= 30-f+food[i+1]*2; n++){
