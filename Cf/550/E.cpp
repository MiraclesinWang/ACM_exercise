...
struct BigInteger{
    ...
    void cutLeadingZero(){while (num.back()==0&&num.size()!=1){num.pop_back();}}
    void setLength(){
        cutLeadingZero();
        int tmp=num.back();
        if (!tmp){length=1;}
    ...
    BigInteger operator/(const long long &b)const{
        ...
        for (int i=num.size()-1;c.num[i]==0;i--) c.num.pop_back();
        return c;
    }
    BigInteger operator/(const BigInteger &b)const{
...
int main()
{
	...
	int n;
	BigInteger num1,num2,res,temp=1,t,m;
	scanf("%d",&n);
	scanf("\n%s",&s1);
	scanf("\n%s",&s2);
	for(int i=n-1;i>=0;i--) t=(s1[i]-'a'),num1+=t*temp,temp*=26;
	temp=1;
	for(int i=n-1;i>=0;i--) t=(s2[i]-'a'),num2+=t*temp,temp*=26;
	temp=1;
	res=(num1+num2)/2;
	res+=temp,res-=temp;
	temp=26;
	for(int i=n-1;i>=0;i--){
		m=(res%temp);
		for(int j=0;j<26;j++){
			if(m==dex[j]) {ans[i]=letter[j];break;}
		}
		res/=temp;
	}
	cout<<ans<<endl;
